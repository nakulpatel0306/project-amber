 ...
 *
 * TypeScript port of the backend's archetype_compatibility.py.
...
export type EmployerArchetypeName = (typeof EMPLOYER_ARCHETYPES)[number];

/**
 * 8x8 matrix: ARCHETYPE_MATRIX[candidateKey][employerName]
 */
const ARCHETYPE_MATRIX: Record<string, Record<string, CompatibilityEntry>> = {
  the_innovator: {
  ...
  the_harmonizer: {
    ...
    'The Visionary': { bonus: 1, synergy_note: 'Ensures people aren\'t left behind in change', friction_note: 'May slow ambitious transformations' },
...
/**
 * Look up compatibility for a single candidate archetype + employer archetype pair.
 */
export function getArchetypeCompatibility(
  candidateKey: string,
  employerName: string,
): CompatibilityEntry {
  const candidateEntry = ARCHETYPE_MATRIX[candidateKey];
  if (!candidateEntry) {
    return { bonus: 0, synergy_note: 'Neutral compatibility', friction_note: 'No specific friction identified' };
  }
  return candidateEntry[employerName] || { bonus: 0, synergy_note: 'Neutral compatibility', friction_note: 'No specific friction identified' };
}

/**
 * Get all 8 employer pairings for a candidate archetype, sorted by bonus (best first).
 */
export function getAllCompatibilityForCandidate(
  candidateKey: string,
): (CompatibilityEntry & { employerArchetype: string })[] {
  const candidateEntry = ARCHETYPE_MATRIX[candidateKey];
  if (!candidateEntry) return [];

  return EMPLOYER_ARCHETYPES.map((name) => ({
    employerArchetype: name,
    ...candidateEntry[name],
  })).sort((a, b) => b.bonus - a.bonus);
}

/**
 * Get all 8 candidate pairings for an employer archetype, sorted by bonus (best first).
 */
export function getAllCompatibilityForEmployer(
  employerName: string,
): (CompatibilityEntry & { candidateArchetype: string; candidateKey: string })[] {
  const candidateKeys = Object.keys(ARCHETYPE_MATRIX);

  return candidateKeys.map((key) => {
    const entry = ARCHETYPE_MATRIX[key]?.[employerName] || {
      bonus: 0,
