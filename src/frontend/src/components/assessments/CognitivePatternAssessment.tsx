...
import { cognitivePatternQuestions } from '../../data/assessmentQuestions';
import { PersonalityEngine, type OCEANScores } from '../../lib/personalityEngine';

const ACCENT = '#EC4899';

export function CognitivePatternAssessment() {
  const { user } = useAuth();
  ...
  const question = cognitivePatternQuestions[currentQuestion];
  const progress = ((currentQuestion + (answers[question?.id] !== undefined ? 1 : 0)) / cognitivePatternQuestions.length) * 100;
  ...
  const handleNext = () => {
    if (currentQuestion < cognitivePatternQuestions.length - 1) {
  ...
  const handleComplete = async () => {
    ...
    for (const q of cognitivePatternQuestions) {
    ...
    if (user) {
      try {
        ...
        showSuccess('Assessment Complete!', 'Your cognitive patterns have been added to your profile.');
      } catch (err) {
        console.error('Error saving cognitive patterns data:', err);
      }
    }
      ...
      <div className="min-h-screen py-8 px-4" style={{ backgroundColor: 'var(--color-background)' }}>
        <div className="max-w-2xl mx-auto">
          <div className="p-8 rounded-2xl border text-center" style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            ...
            <h1 className="text-2xl font-bold mt-4 mb-2" style={{ color: 'var(--color-text)' }}>Cognitive Patterns Results</h1>
            <p className="text-sm mb-8" style={{ color: 'var(--color-textSecondary)' }}>
              Here's what your thinking style reveals about your cognitive approach
            </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: 'var(--color-textMuted)' }}>
                    Your cognitive patterns have been factored into your personality profile with a 10% weight, making your matches more nuanced.
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold" style={{ color: 'var(--color-text)' }}>Cognitive Patterns Assessment</h1>
              <p className="text-xs" style={{ color: 'var(--color-textMuted)' }}>
                Question {currentQuestion + 1} of {cognitivePatternQuestions.length}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            rightIcon={currentQuestion === cognitivePatternQuestions.length - 1 ? <CheckCircle2 className="w-4 h-4" /> : <ArrowRight className="w-4 h-4" />}
          >
            {currentQuestion === cognitivePatternQuestions.length - 1 ? 'Complete' : 'Next'}
          </Button>
        </div>
      </div>
    </div>
