...
import { workEnvironmentQuestions } from '../../data/assessmentQuestions';
import { PersonalityEngine, type OCEANScores } from '../../lib/personalityEngine';

const ACCENT = '#06B6D4';

export function WorkEnvironmentAssessment() {
  const { user } = useAuth();
  ...
  const question = workEnvironmentQuestions[currentQuestion];
  const progress = ((currentQuestion + (answers[question?.id] !== undefined ? 1 : 0)) / workEnvironmentQuestions.length) * 100;
  ...
  const handleNext = () => {
    if (currentQuestion < workEnvironmentQuestions.length - 1) {
  ...
  const handleComplete = async () => {
    ...
    for (const q of workEnvironmentQuestions) {
    ...
    if (user) {
      try {
        ...
        const { data: assessmentData, error: assessmentError } = await supabase
          ...
          .insert({
            user_id: user.id,
            assessment_type: 'work_environment',
            responses: answers,
        ...
        showSuccess('Assessment Complete!', 'Your work environment profile has been added.');
      } catch (err) {
        console.error('Error saving work environment data:', err);
      }
    }
  ...
  if (isComplete && oceanScores) {
    ...
    return (
      <AssessmentResultsTemplate
        title="Work Environment & Pace Results"
        subtitle="Here's how your work environment and pace preferences shape your culture"
        dimensions={dimensions}
        enhancedMessage="Your work environment preferences have been factored into your culture profile with a 10% weight, making your candidate matches more accurate."
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold" style={{ color: 'var(--color-text)' }}>Work Environment & Pace</h1>
              <p className="text-xs" style={{ color: 'var(--color-textMuted)' }}>
                Question {currentQuestion + 1} of {workEnvironmentQuestions.length}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            rightIcon={currentQuestion === workEnvironmentQuestions.length - 1 ? <CheckCircle2 className="w-4 h-4" /> : <ArrowRight className="w-4 h-4" />}
          >
            {currentQuestion === workEnvironmentQuestions.length - 1 ? 'Complete' : 'Next'}
          </Button>
        </div>
      </div>
    </div>
