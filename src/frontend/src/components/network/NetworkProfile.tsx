  ...
  const handleSeedTestData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        loadCatalogData(); // Reload to show new data
      } else {
        ...
        <div className="flex items-start gap-5 flex-wrap sm:flex-nowrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            {catalogData.location && (
              <div className="flex items-center gap-1.5 text-sm mb-3" style={{ color: 'var(--color-textMuted)' }}>
                <MapPin className="w-4 h-4" />
                {catalogData.location}
              </div>
            )}
            {catalogData.bio && (
              <p className="text-sm line-clamp-3" style={{ color: 'var(--color-textSecondary)' }}>
                {catalogData.bio}
              </p>
            )}
          </div>
          <div className="flex flex-col gap-2">
            ...
            {import.meta.env.DEV && (
              <Button
                variant="ghost"
                size="sm"
                leftIcon={isSeeding ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bento-card p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <ImageIcon className="w-5 h-5" style={{ color: 'var(--color-accent)' }} />
              <h3 className="font-semibold" style={{ color: 'var(--color-text)' }}>
                Photo Gallery
              </h3>
              <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: 'var(--color-background)', color: 'var(--color-textMuted)' }}>
          ...
          {catalogData.photos.length > 0 ? (
            ...
            <div
              className="border-2 border-dashed rounded-xl p-6 text-center cursor-pointer hover:border-[var(--color-accent)] transition-colors"
              style={{ borderColor: 'var(--color-border)' }}
              onClick={() => setShowPhotoModal(true)}
            >
              <Camera className="w-6 h-6 mx-auto mb-2" style={{ color: 'var(--color-textMuted)' }} />
        ...
        <div className="bento-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              {catalogData.hobbies.length > 0 ? 'Edit' : 'Add'}
            </Button>
          </div>
          ...
          {selectedHobbies.length > 0 ? (
            ...
            >
              <Sparkles className="w-6 h-6 mx-auto mb-2" style={{ color: 'var(--color-textMuted)' }} />
              <p className="text-xs" style={{ color: 'var(--color-textMuted)' }}>
                Add hobbies to express your personality
              </p>
            </div>
          )}
        </div>
